...
function getRegion() {
	...
	for (var i = 0; i < city[prov].length; i++) {
		$("#city").append("<option value=" + city[prov][i] + ">" + city[prov][i] + "</option>"); // 城市
	}
}
function getProvince() {
	for (var i = 0; i <= 33; i++) {
		$("#province").append("<option value=" + i + ">" + city[i][0] + "</option>"); // 省份
	}
}
/* 设置默认城市 */
function setCity(i, j) {
	$("#province option[value=" + i + "]").attr("selected", "selected");
	getRegion();
	$("#city option[value=" + j + "]").attr("selected", "selected");
}